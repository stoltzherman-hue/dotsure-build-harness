...
export function Concierge() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setApiError(true)
      setMsgs(m => [...m, { role: "assistant", content: "Connection error. Check that ANTHROPIC_API_KEY is set in Vercel environment variables." }])
    }
    setLoading(false)
  }
      ...
      {open && (
        <div style={{ position: "fixed", bottom: 80, right: 20, width: 340, background: "var(--wh)", borderRadius: 16, boxShadow: "0 8px 40px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", maxHeight: 480, zIndex: 1000 }}>
          <div style={{ background: "var(--g900)", padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, borderRadius: "16px 16px 0 0" }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--org)" }} />
            <span style={{ color: "white", fontSize: 12, fontWeight: 600, flex: 1 }}>Governance Concierge</span>
            <button onClick={() => setOpen(false)} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 16, lineHeight: 1 }}>×</button>
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {loading && <div style={{ fontSize: 11, color: "var(--g400)", fontStyle: "italic" }}>Thinking...</div>}
            {apiError && <div style={{ fontSize: 10, color: "#dc2626", padding: "4px 8px", background: "#fef2f2", borderRadius: 6 }}>AI service unavailable. Ensure ANTHROPIC_API_KEY is set in Vercel environment variables.</div>}
            <div ref={ref} />
          </div>
